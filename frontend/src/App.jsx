            ...
            <header style={{ padding: '60px 0 40px', textAlign: 'center' }} className="animate-fade">
                <div className="container">
                    <div className="logo-container">
                        {settings.siteLogo ? (
                            <img src={`${API_FILE_URL}${settings.siteLogo}`} alt="Logo" />
                        ) : (
                            <div style={{ background: 'var(--primary)', width: '100%', height: '100%', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '2rem', fontWeight: 800 }}>P</div>
                        )}
                    </div>
                    <p style={{ color: 'var(--gray)', fontSize: '0.95rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '10px' }}>
                        {settings.heroTitle || 'Personalizados para você'}
                    </p>
                    <h1 style={{ fontSize: '1.4rem', color: 'var(--secondary)', maxWidth: '600px', margin: '0 auto 20px', lineHeight: 1.4 }}>
    ...
    useEffect(() => {
        getSettings().then(res => {
            ...
            if (res.data.primaryColor) {
                ...
                // Optional: set a lighter version for shades
            }
        });
    }, []);
